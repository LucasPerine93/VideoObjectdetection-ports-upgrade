...
class VideoObjectDetection:
    ...
    def __init__(
        ...
        port: int = 4912,
        stream_port: int = 5050,
        service: str = "ei-video-obj-detection-runner1",
    ):
        ...
        if self.service:
            if self.service not in infra["services"]:
                raise ValueError(f"Service '{self.service}' not found in brick_compose.yaml")
            self._host = self.service
        else:
            self._host = next(iter(infra["services"].key()))
        ...
        while self._is_running.is_set():
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as tcp_socket:
                    tcp_socket.connect((self._host, self._stream_port))
